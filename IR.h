...
int read_ir(byte i2c_id=0x01) {
  ...
  Wire.requestFrom(i2c_id, 1);    // reques 1 bytes from slave device 0x01
  ...
  Wire.requestFrom(i2c_id, 1);    // request 6 bytes from slave device #2
  while (Wire.available())   // slave may send less than requested
  {
